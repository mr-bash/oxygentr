...
class about extends mysql_connect {
  ...
  private function check_type() {
    ...
    if($this->type_post == 'add') {
      if($this->add_about()) {
        ...
        $this->redirecr_page(messages::$msg_alert, 'about.php', 2);
        exit();
      }
      return false;
    } else {
  ...
  private function add_about() {
    $upload   = new upload();

    if($this->chek_input() and $upload->up($this->image, $this->type_post, $this->name_dir)) {
      $add = $this->insert('about', 'title, content, img', ':xtitle, :xcontent, :ximg');
      $add_array = array(
  ...
  private function edit_about() {
    $database = $this->select('*', 'about', 'WHERE id = ?');
    $database->execute(array($this->get_id));
    $data = $database->fetch();
    $upload   = new upload();
    if($this->chek_input() and $upload->up($this->image, $this->type_post, $this->name_dir)) {
      if($this->image['size']   == 0 ) { $upload->end_name_file   = $data['img']; }
      $edit = $this->update('about', 'title = ?, content = ?, img = ?', 'WHERE id = ?');
