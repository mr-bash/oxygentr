                        ...
                        <tbody>
                          <?php
                            ...
                            if(isset($_GET['del']) and is_numeric($_GET['del'])) {
                              $id = $_GET['del'];
                              $all = $DB->select('sec_id', 'works_sections', 'WHERE sec_id = ?');
                              $all->execute(array($_GET['del']));
                              $res = $all->rowCount();
                              if($res > 0) {
                                $del = $DB->delete('works_sections', 'WHERE sec_id = ?');
                                $del->execute(array($_GET['del']));
                                $DB->redirecr_page(' ', $_SERVER['HTTP_REFERER'], 0);
                              } else {
                                $DB->redirecr_page(' ', 'sections_works.php', 0);
                              }
                            } else {
                              $id = 0;
                            }

                            $select_section = $DB->select('*', 'works_sections');
                            ...
                            <tr class="text-center custom-td">
                                ...
                                <td><a href="edit_section_works.php?id=<?php echo $data['sec_id']; ?>" class="btn btn-sm btn-warning">تعديل</a></td>
                                <td><a href="<?php echo '?del=' . $data['sec_id']; ?>" class="btn btn-sm btn-danger confirm_important">حذف</a></td>
                            </tr>
                            <?php } ?>
                        </tbody>
                    </table>
                    <a href="add_section_works.php" class="btn-add"><i class="fas fa-plus-circle"></i> أضافة</a>
                  </div>
              </div>
          </div>
      </div>
